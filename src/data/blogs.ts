export const blogs = [
    {
        id: "1",
        title: "10 Creative Ways to Use Fresqo Cocktail Balls at Your Next Party",
        excerpt: "Discover unique serving ideas and creative combinations to elevate your hosting game with our premium cocktail balls.",
        content: "Hosting a party? The right drink can turn a good event into a great one. With Fresqo Cocktail Balls, you have a versatile, delicious, and engaging addition to any gathering. Here are 10 creative ways to use them:\n\n1. The Classic Drop: Simple, elegant. Drop a ball into your favorite spirit or sparkling water and watch the magic happen.\n2. Layered Visuals: Create a layered effect in tall glasses by using multiple flavors.\n3. Ice Tray Enhancements: Freeze half a ball in an ice cube for a delayed flavor release.\n4. Garnish Galore: Use the balls alongside traditional garnishes for an extra pop of color.\n5. Theme Alignment: Match the colors to your party theme.\n6. DIY Drink Station: Set up a station where guests pick their favorite ball and mixer.\n7. Dessert Enhancements: They aren't just for drinks; use them in creative dessert presentations.\n8. Welcome Drinks: A refreshing greeting for your guests.\n9. Pairings: Pair certain flavors with specific appetizers.\n10. Take-home Favors: Send guests home with a beautifully packaged cocktail ball.",
        date: "Oct 15, 2023",
        readTime: "5 min read",
        image: "/images/hero-bomb.jpg",
        category: "Entertaining",
    },
    {
        id: "2",
        title: "The Science Behind the Perfect Fizz",
        excerpt: "Ever wondered what makes our cocktail balls fizz so perfectly? Dive into the science of carbonation and flavor release.",
        content: "At Fresqo, the fizz isn't just for show—it's essential to the experience. When one of our cocktail balls hits the liquid, a carefully balanced reaction occurs. This effervescence helps to release the concentrated flavors more effectively. It creates a bubbling visual effect while the aroma is distributed alongside the drink, engaging your sense of smell before you even take a sip.",
        date: "Oct 28, 2023",
        readTime: "4 min read",
        image: "/images/product-cola.jpg",
        category: "Behind the Scenes",
    },
    {
        id: "3",
        title: "Mocktail Magic: Crafting Alcohol-Free Masterpieces",
        excerpt: "Who says you need alcohol to have fun? Learn how to mix incredible mocktails using Fresqo for the whole family.",
        content: "You don't need a heavy pour of spirits to craft an unforgettable drink. Fresqo Cocktail Balls are perfect for creating dynamic non-alcoholic beverages. Simply pair them with sparkling water, tonic, or fruit juice. The intense flavor profile ensures that the resulting drink is anything but boring. Try our Mojito flavor in a glass of soda water with fresh mint—it's refreshing and entirely alcohol-free!",
        date: "Nov 05, 2023",
        readTime: "6 min read",
        image: "/images/product-mojito.jpg",
        category: "Recipes",
    },
    {
        id: "4",
        title: "Pairing Fresqo Flavors with Gourmet Appetizers",
        excerpt: "A comprehensive guide to matching our delicious cocktail and mocktail flavors with the perfect bite-sized party foods.",
        content: "A great drink deserves great food. Pairing flavors can be tricky, but Fresqo makes it easy. For example, our Kala Khatta flavor pairs wonderfully with spicy snacks, like samosas or jalapeño poppers, balancing the heat. Our Pinacolada flavor, with its sweet coconut notes, is the perfect match for shrimp or tropical appetizers.",
        date: "Nov 12, 2023",
        readTime: "7 min read",
        image: "/images/product-watermelon.jpg",
        category: "Pairings",
    },
    {
        id: "5",
        title: "From Setup to Cleanup: The Ultimate Host's Guide",
        excerpt: "Streamline your party planning with our foolproof guide to hosting an unforgettable event with minimal stress.",
        content: "Hosting can be stressful, but it doesn't have to be. With a bit of preparation, you can enjoy your own party. One key tip: prep your drinks ahead of time. Featuring a Fresqo DIY drink station means guests serve themselves, freeing you up to mingle. Keep plenty of ice on hand and ensure your cocktail balls are beautifully displayed!",
        date: "Nov 20, 2023",
        readTime: "5 min read",
        image: "/images/product-pinacolada.jpg",
        category: "Entertaining",
    }
];
